use crate::{bitmap::Bitmap, memory::GetPages};
...
impl PageFrameAllocator {
    ...
    pub fn init(&mut self, boot_info: &boot::Info, k_start: u64, k_end: u64) {
        ...
        self.iter_action(Self::lock_page,self.page_bitmap.addr as usize, self.page_bitmap.size / boot::PAGE_SIZE + 1); 
        self.iter_action(Self::lock_page, k_start as usize, (k_end - k_start) as usize / boot::PAGE_SIZE + 1);
        self.iter_action(Self::lock_page, boot_info.graphics.frame_buffer_base_address as usize, boot_info.graphics.frame_buffer_size/ boot::PAGE_SIZE +1);
    ...
    pub fn lock_page(&mut self, page_addr: usize) -> bool {
        if self.page_bitmap[page_addr / boot::PAGE_SIZE] || !self.page_bitmap.set(page_addr / boot::PAGE_SIZE, true) {
            return false;
        }
