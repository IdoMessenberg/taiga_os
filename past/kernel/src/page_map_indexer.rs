...
impl PageTableManager {
    ...
    pub fn map_memory(&self,virtual_address: u64, physical_address: u64, con_out: &mut console::Output) {
        ...
        let page_directory_pointer: *mut PageTable = if page_directory_entry.get_flag(PageTableFlag::Present) {
            ...
            let address = unsafe {crate::GLOBAL_ALLOC.get_free_page(con_out).expect("no free pages")};
        ...
        page_directory_entry = unsafe {(*page_directory_pointer).entries[indexer.page_directory_pointer_index as usize]};
        let page_directory_pointer: *mut PageTable = if page_directory_entry.get_flag(PageTableFlag::Present) {
            (page_directory_entry.get_address() << 12) as *mut PageTable
        }
        else{
            ...
            page_directory_entry.set_flag(PageTableFlag::ReadWrite, true);
            unsafe{
               (*page_directory_pointer).entries[indexer.page_directory_pointer_index as usize] = page_directory_entry;
            }
            address as *mut PageTable
        };
        page_directory_entry = unsafe {(*page_directory_pointer).entries[indexer.page_table_index as usize]};
        let page_table: *mut PageTable = if page_directory_entry.get_flag(PageTableFlag::Present) {
            ...
            page_directory_entry.set_flag(PageTableFlag::ReadWrite, true);
            unsafe{
                (*page_directory_pointer).entries[indexer.page_table_index as usize] = page_directory_entry;
            }
            address as *mut PageTable
        };
