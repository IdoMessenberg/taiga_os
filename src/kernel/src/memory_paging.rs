use crate::{boot, data_types::Bitmap, memory_map};
...
impl PageFrameAllocator {
    pub const fn empty() -> Self { PageFrameAllocator { total_mem: 0, used_mem: 0, free_mem: 0, resv_mem: 0, initialised: false, page_bitmap: Bitmap { size: 0, address: 0 } } }
    pub fn initialise(&mut self, boot_info: &boot::Info) {
        ...
        self.lock_pages(boot_info.graphics.frame_buffer_base_address, boot_info.graphics.frame_buffer_size as usize / 0x1000 + 1);

        for i in 0..(boot_info.mem_map_info.size / boot_info.mem_map_info.descriptor_size - 2) {
            let memory_map_segment_descriptor: &memory_map::Descriptor = unsafe { &*((boot_info.mem_map_info.address + i as u64 * boot_info.mem_map_info.descriptor_size as u64) as *const memory_map::Descriptor) };
