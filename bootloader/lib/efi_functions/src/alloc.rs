...
#[global_allocator]
static mut BOOT_TIME_EFI_ALLOCATOR: BootTimeEfiAllocator = BootTimeEfiAllocator::const_default();
...
impl BootTimeEfiAllocator {
    const fn const_default() -> Self { BootTimeEfiAllocator { alloc_pool: None, free_pool: None } }
...
pub trait BootTimeAllocFunctions {
    ...
    fn get_memory_map(&self) -> Result<uefi::MemoryMapInfo, Status>;
}
...
impl BootTimeAllocFunctions for boot_time::Services {
    ...
    fn get_memory_map(&self) -> Result<uefi::MemoryMapInfo, Status> {
        let memory_map = uefi::MemoryMapInfo::default();
