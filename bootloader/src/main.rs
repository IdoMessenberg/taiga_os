...
extern "C" fn main(handle: *const core::ffi::c_void, system_table: efi::system::Table) -> efi::Status {
    ...
    let font:  efl::psf::FontInfo = get_psf_font_info(&system_table, system_root, &config);
    let mut graphics_info: efi::graphics::Info = gop.get_graphics_info();
    graphics_info.theme = config.graphics_theme;

    let memory_map: efi::alloc::MemoryMapInfo = match system_table.boot_time_services.get_memory_map() {
        Ok(mem_map) => mem_map,
    ...
    kernel_entry_point(boot::Info { graphics: graphics_info, font, memory_map_info: memory_map});
...
fn init(system_table: &efi::system::Table) {
    ...
    match efi::alloc::init(system_table) {
...
fn get_system_root<'a>(handle: *const core::ffi::c_void, system_table: &'a efi::system::Table) -> &'a efi::protocols::media_access::file::Protocol{
    match efi::file::get_root(handle, system_table.boot_time_services) {
        Ok(root) => {
